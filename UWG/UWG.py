...
class UWG(object):
    ...
    def read_epw(self):
        ...
        try:
            ...
            raise Exception("Failed to read epw file! {}".format(e))
    ...
    def read_input(self):
        ...
        try:
            ...
            raise Exception("Failed to read .uwg file! {}".format(e))
        ...
        while  count < len(uwg_param_data):
            ...
            if row == [] or "#" in row[0]:
                ...
                self._init_param_dict[row[0]] = list(map(lambda r: Utilities.str2fl(r[:24]),trafficrows))
                count += 4
            elif row[0] == "bld":
                #bld: 17 x 3 matrix
                bldrows = uwg_param_data[count+1:count+17]
                self._init_param_dict[row[0]] = list(map(lambda r: Utilities.str2fl(r[:3]),bldrows))
                count += 17
            elif row[0] == "LAD":
                # LAD profile
                LADrows = uwg_param_data[count+1:count+3]
                self._init_param_dict[row[0]] = list(map(lambda r: Utilities.str2fl(r[:(len(LADrows[0])-1)]),LADrows))
                count += 3
    ...
    def instantiate_input(self):
        ...
        thickness_vector = list(map(lambda r: 0.05, range(road_layer_num)))
        material_vector = list(map(lambda n: asphalt, range(road_layer_num)))
