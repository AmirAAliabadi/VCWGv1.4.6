...
class Building(object):
    ...
    def BEMCalc(self, UCM, BEM, forc, parameter, simTime, bx, by,
        ...
        if self.sensCoolDemand > 0. and UCM.canTemp > 288.:
            ...
            # Set dehumidification demand as a fraction of sensible cooling demand
            # Consider the summation of air conditioning and heat pump contributions
            self.dehumDemand = 0.1 * (self.sensCoolDemand + self.Q_hp)

            # Save dehumidification demand for later use because it may be modified
            Qdehum = self.dehumDemand
        ...
        if Adv_ene_heat_mode == 0:
            Q = self.intHeat + winTrans + self.Qheat - self.sensCoolDemand - self.Q_hp
        if Adv_ene_heat_mode == 1:
            Q = self.intHeat + winTrans + self.Qheat - self.sensCoolDemand + self.Q_hp
